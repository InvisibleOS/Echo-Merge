...
export default function CaseQueue({ cases, onUpdateStatus }: Props) {
  return (
    <section className="rounded-lg border border-white/10 bg-ink-900/70 p-4 min-h-[360px]">
      ...
      <div className="space-y-3 max-h-[520px] overflow-y-auto pr-1">
        {cases.slice(0, 12).map((item) => (
          <article key={item.case_id} className="rounded-md bg-white p-3 border border-ink-900/10">
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-md bg-civic-600 px-3 py-1.5 text-xs font-semibold text-white"
              >
                <Send size={12} /> Assign
              </button>
              <button
                ...
                className="inline-flex items-center gap-1 rounded-md border border-ink-900/10 px-3 py-1.5 text-xs font-semibold text-ink-800"
              >
                <FileCheck2 size={12} /> Resolve
              </button>
            </div>
          </article>
        ))}
