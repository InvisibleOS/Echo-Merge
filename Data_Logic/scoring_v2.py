...
class ScoringPipelineV2:
    ...
    def calculate_priority_score_v2(self, cluster: dict) -> dict:
        ...
        # routine/low=0.05, medium=0.12, high=0.18, critical=0.25
        urgency_weights = {
            "Routine": 0.0,
            "Low": 0.05,
            "Medium": 0.12,
        ...
        if "school" in category_lower or "education" in category_lower:
            ...
            # Cross-reference with hospital bed gap
            gap_ratio = demo.get("hospital_bed_gap", 0.30)
            public_data_gap = gap_ratio * 0.20
        elif "garbage" in category_lower or "sanitation" in category_lower:
            # Cross-reference with waste treatment gap
            gap_ratio = demo.get("waste_treatment_gap", 0.35)
            public_data_gap = gap_ratio * 0.20
            
        # 6. Feasibility (0.0 to 0.15)
        # Boost if category aligns with municipal budget or is news corroborated
        feasibility = 0.05
        # Standard BBMP budget areas: roads, streetlights, waste
    ...
    def run_priority_generation_v2(self) -> list:
        """Executes the full scoring and ranking pipeline under v2 specs."""
