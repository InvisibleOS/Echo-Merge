...
function computeLiveScoringBreakdown(demandCount, urgency, category, existingBreakdown = null) {
  ...
  const validation_multiplier = existingBreakdown?.validation_multiplier || 0.15;
  ...
  return {
    ...
    reasoning: {
      ...
      validation: existingBreakdown?.reasoning?.validation || `Live AI validation heuristics confirm alignment with known infrastructural gap patterns.`
    }
  };
}
...
export async function rescoreForSubmission(supabase, { enriched, submissionId, rawSubmission }) {
  ...
  if (existing) {
    ...
    const breakdown = computeLiveScoringBreakdown(newCount, enriched.urgency, category, existing.scoring_breakdown);
    ...
    const breakdown = computeLiveScoringBreakdown(1, enriched.urgency, category);
