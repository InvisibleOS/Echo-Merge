...
export function rateComplaint(signals = {}) {
  ...
  const urgencyLabel = resolveUrgency(signals.urgency, signals.demand_score);
  const { feasibility, cost } = categoryProfile(signals.category);

  return finalize({
    urgency: URGENCY_SCORE[urgencyLabel] ?? 3,
