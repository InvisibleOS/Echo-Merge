...
export function getActionPriorities({ constituency = '', sortBy = 'rank' } = {}) {
  ...
  const all = [...livePriorities, ...base]
    .filter((item) =>
      constituency ? (item.hotspot_geo?.ward || item.constituency || '').includes(constituency) : true
    )
    .filter((item) => !item.title?.includes('[PROACTIVE'));

  // Sync statuses from cases and overrides to priorities
  const caseStatusMap = {};
  for (const c of liveCases) {
    caseStatusMap[c.work_id] = c.status;
  }
  const s = store();
  for (const [caseId, override] of Object.entries(s.caseOverrides)) {
    if (override.status) {
      const matchingItem = all.find((item) => {
        if (String(item.work_id).startsWith('LIVE-')) {
          const idPart = String(item.work_id).replace(/^LIVE-/, '');
          const expectedCaseId = `CASE-${idPart.replace(/^SUB-/, '')}`;
          return expectedCaseId === caseId;
        }
        const expectedCaseId = `CASE-${String(item.rank).padStart(3, '0')}`;
        return expectedCaseId === caseId;
      });
      if (matchingItem) {
        caseStatusMap[matchingItem.work_id] = override.status;
      }
    }
  }

  for (const item of all) {
    const caseStatus = caseStatusMap[item.work_id];
    if (caseStatus) {
      item.solution_plan = {
        ...(item.solution_plan || {}),
...
function priorityFromCase(caseItem, rank) {
  return {
    ...
    solution_plan: {
      resolved: caseItem.status === 'Resolved'
    },
  };
}
