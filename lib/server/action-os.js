...
import nationwideBackup from '../../Data_Logic/local_db_backup.json';
import { enrich } from './enrichment.js';

const DEFAULT_CONSTITUENCY = 'Bengaluru South';
const NOW = () => new Date();
const nationwideSubmissions = Object.values(nationwideBackup.submissions || {});
const prioritySeed = nationwideBackup.priorities?.length ? nationwideBackup.priorities : rawPriorities;
const submissionSeed = nationwideSubmissions.length ? nationwideSubmissions : rawSubmissions;
...
export function getActionHotspots({ constituency = '' } = {}) {
  ...
  const historical = submissionSeed
    ...
    .map((submission) => ({
      ...
      constituency: submission.constituency || submission.geo.ward || DEFAULT_CONSTITUENCY,
      intensity: urgencyIntensity(submission.urgency),
      category: submission.category,
      demand_count: 1,
    }));
...
export function getActionSubmissions() {
  return [...store().submissions, ...submissionSeed];
}
...
function matchesConstituency(item, constituency) {
  return [
    item.constituency,
    item.state,
    item.hotspot_geo?.ward,
    item.geo?.ward,
  ]
    .filter(Boolean)
    .some((value) => String(value).includes(constituency));
}
