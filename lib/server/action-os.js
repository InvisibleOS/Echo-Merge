...
export function updateCaseStatus(caseId, status, note = '') {
  const s = store();
...
export async function askCopilot(question = '', { constituency = DEFAULT_CONSTITUENCY } = {}) {
  ...
  let answer;
  if (/water|supply|drain/.test(q)) {
    const water = priorities.filter((p) => p.category === 'Water Supply and Services').slice(0, 3);
    answer = water.length
      ? `Water is concentrated in ${water.map((p) => p.hotspot_geo.ward || constituency).join(', ')}. The highest scoring water cluster is "${water[0].title}" with score ${water[0].demand_score}.`
      : 'No major water cluster is currently above the watch threshold.';
  } else if (/sla|delay|overdue/.test(q)) {
    const risky = departments.sort((a, b) => b.sla_breaches - a.sla_breaches)[0];
    answer = `${risky.name} has the highest SLA risk with ${risky.sla_breaches} breaches and ${risky.active_cases} active cases. Recommended action: ${risky.recommended_action}`;
  } else if (/budget|fund|money/.test(q)) {
    const budget = buildBudgetRecommendations(priorities)[0];
    answer = `${budget.category} is the best budget candidate. Suggested tier: ${budget.recommended_budget_tier}. Rationale: ${budget.rationale}`;
  } else if (/trust|health|score/.test(q)) {
    answer = `${constituency} health index is ${health.health_index}/100 and citizen trust score is ${health.citizen_trust_score}/100. The main pressure point is ${health.top_issue}.`;
  } else {
    const top = priorities[0];
    answer = top
      ? `Focus first on "${top.title}". It has ${top.demand_count} supporting citizen reports, score ${top.demand_score}, and should be routed to ${departmentForCategory(top.category).name}.`
      : 'No priority data is available yet.';
  }

  const fallback = {
    question,
    answer,
    citations: priorities.slice(0, 3).map((p) => ({
      work_id: p.work_id,
      title: p.title,
      score: p.demand_score,
    })),
    suggested_actions: [
      'Open the top cluster and assign the owner department.',
      'Ask the department for a 24-hour field verification update.',
      'Publish a public status note for citizen trust.',
    ],
  };
  ...
  try {
    const system = `You are an AI Copilot for an Indian MP/MLA. Answer using only the provided constituency data.
Be concise, operational, and action-oriented. Return JSON with answer, suggested_actions, and citations.`;
    const prompt = JSON.stringify(
      {
        question,
        constituency,
        health,
        top_priorities: priorities.slice(0, 8).map((p) => ({
          work_id: p.work_id,
          title: p.title,
          category: p.category,
          score: p.demand_score,
          demand_count: p.demand_count,
          department: departmentForCategory(p.category).short_name,
    ...
    const ai = await generateGeminiJson({ system, prompt, temperature: 0.15 });
    if (!ai || !ai.answer) return fallback;
    return {
      question,
      answer: String(ai.answer),
      citations: Array.isArray(ai.citations) && ai.citations.length ? ai.citations : fallback.citations,
      suggested_actions:
        Array.isArray(ai.suggested_actions) && ai.suggested_actions.length
          ? ai.suggested_actions.map(String)
          : fallback.suggested_actions,
    };
  } catch (error) {
    console.error('[copilot] Gemini failed, using fallback:', error.message);
    return fallback;
  }
}
