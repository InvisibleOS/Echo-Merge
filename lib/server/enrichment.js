...
import { googleWebSearch } from './googleSearch.js';
...
// Cross-reference the claim against the live web (Google Custom Search) so the
// Gemini enrichment can flag contradicted/false claims. Returns a snippet blob
// or null when search is unconfigured / finds nothing.
async function performWebSearch(query) {
  const results = await googleWebSearch(query, { num: 3 });
