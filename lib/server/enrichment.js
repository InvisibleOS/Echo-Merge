...
// slow Nominatim round-trips on the awaited /submit critical path.
...
async function reverseGeocode(lat, lng) {
  ...
  try {
    const res = await fetchWithTimeout(
      ...
      { headers: { 'User-Agent': 'EchoMerge/1.0 (hackathon-demo)' } },
      4000
    );
    const data = await res.json();
    if (data && data.address) {
      result = data.address.neighbourhood || data.address.suburb || data.address.village || data.address.town || data.address.city || data.display_name.split(',')[0];
    }
  } catch (e) {
