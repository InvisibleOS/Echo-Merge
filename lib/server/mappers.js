...
import { buildOptimalSolution } from './solution.js';
...
function toEvidence(e) {
  ...
  // Media presence flags (the bytes are fetched lazily by submission_id).
  if (e.has_photo) out.has_photo = true;
  if (e.has_audio) out.has_audio = true;
  return out;
}
...
export function toPriorityItem(row) {
  ...
  // Every complaint gets an optimal AI solution + government budget estimate.
  // A real planner's DB solution_plan (and its `resolved` flag) always wins;
  // the generated plan fills the gaps so no issue is left without one.
  const { solution_plan: generatedPlan, resolution_brief } = buildOptimalSolution({
    title: row.title,
    category: row.category,
    demand_count,
    demand_score,
  ...
  const dbPlan = row.solution_plan && typeof row.solution_plan === 'object' ? row.solution_plan : {};
  const solution_plan = { ...generatedPlan, ...dbPlan };
  ...
  return {
    ...
    solution_plan,
    resolution_brief,
    status: row.solution_plan?.resolved
      ? 'Resolved'
      : row.solution_plan?.assigned
        ? 'Assigned'
        : 'In Progress',
    assigned_department: row.solution_plan?.assigned_department ?? undefined,
    // Last-activity timestamp — the delegation queue sorts by this ("most recent"
    // first) so a just-posted, just-converted or just-assigned item leads.
